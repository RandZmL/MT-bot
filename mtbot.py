...
async def stats(ctx, member : discord.Member):
	user_id = '\\' + str(member.mention)
	user_discord_name = str(member).replace('@', '')[:-5]
	cursor = db.cursor()
	information = []
	uuid = []
	...
	try:
		...
		emb.set_author(name = information[4] + ': ' + information[9], icon_url = member.avatar_url)
...
async def bp(ctx, member : discord.Member): 
	...
	bp_level = 0
	bp_type ='free'
	payed_next_reward = 'gay sex'
	free_next_reward = 'Калл'
	...
	try:
		...
		cursor.execute("SELECT name FROM bw_stats_players WHERE uuid = '" + uuid[0] + "'")
		for x in cursor:
			name += x

		cursor.execute("SELECT rang FROM bw_rang_texted WHERE uuid = '" + uuid[0] + "'")
		for x in cursor:
			rang += x
		emb = discord.Embed(title ="Боевой пропуск игрока: " + user_discord_name,description ='Тип боевого пропуска: ' + str(bp_type)  , colour = discord.Color.red())
		emb.set_author(name = name[0] + ': ' + rang[0], icon_url = member.avatar_url)
		emb.set_thumbnail(url = member.avatar_url)
		emb.add_field(name = "Текущий уровень боевого пропуска: " , value = str(bp_level))
		if bp_type == 'payed':
			emb.add_field(name='Награда за следующий уровень:', value=str(payed_next_reward))
		if bp_type == 'free':
			emb.add_field(name='Награда за следующий уровень:', value=str(free_next_reward))
			emb.add_field(name ='У вас нет боевого пропуска. Купить боевой пропуск можно тут:', value='http://scam.com', inline=False)
		await ctx.send(embed = emb)
	except Exception as e:
		print(f'Ошибка при использовании команды .bp {e}')
		await ctx.send('**Ошибка:** пользователь не найден. Возможно, ты допустил ошибку в никнейме, или же такого игрока нет на сервере.')

########################################
####     (временно) Слеш команды    ####
########################################

@slash.slash()                               #Царан (Команда help должна показать пользователю, какие команды он может использовать)
